	...
	class Sensei_Lesson_Timer {
		...
		function template_redirect() {
			...
			if ($queried_object->post_type == 'lesson') {
				...
				$started_course = WooThemes_Sensei_Utils::user_started_course( $course_id, get_current_user_id() );
		...
		function sensei_settings_fields( $fields ) {
			...
			$this->post_type_setting_args = array(
				...
				'data' => array(
					'name'        => __( 'Add Lesson Timer to these Sensei post types', 'sensei-lesson-timer' ),
					'description' => __( 'The Lesson Timer will be displayed on the these Sensei post types', 'sensei-lesson-timer' ),
			...
			$fields['slt_setting_placement'] = array(
				...
				'required'    => 0,
				'options'     => array(
					'outside-right'   => __('Disable Button, Timer after button', 'sensei-lesson-timer'),
					'outside-replace' => __('Hide Button, Show Timer', 'sensei-lesson-timer'),
					'inside-right'    => __('Add Timer after Button Text', 'sensei-lesson-timer'),
				),
			);
		...
		public function admin_enqueue_scripts() {
			wp_enqueue_script(
				...
				$this->plugin_url .'js/sensei-lesson-timer-admin.js',
				array( 'woothemes-sensei-settings' ),
				$this->version,
				true
			);
		...
		public function admin_enqueue_styles() {
			wp_enqueue_style(
				...
				$this->plugin_url .'css/sensei-lesson-timer-admin.css',
				array( 'woothemes-sensei-settings-api' ),
				$this->version
			);
		}
...
add_action('plugins_loaded', function() {
	...
	$is_active = array_filter( $active_plugins, function( $plugin ) {
		return false !== strpos( $plugin, '/woothemes-sensei.php' );
	} );

	if ( ! empty( $is_active ) ) {
		Sensei_Lesson_Timer::get_instance();
	} else {
		trigger_error( __( 'Sensei Lesson Timer requires the Sensei plugin! Please install and activate Sensei (https://github.com/Automattic/sensei).', 'sensei-lesson-timer' ), E_USER_WARNING );
	}
});
